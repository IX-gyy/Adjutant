...
```
**关键优化**：
- **单工具直接执行**：当关键词匹配仅对应一个工具时（如"提醒我"→todo），直接调用工具处理，无需GLM分类器。工具内部已内置GLM解析逻辑（如TodoTool的_classify_todo），减少一次GLM调用。
- **多工具才走分类器**：只有当用户输入同时匹配多个工具关键词时（如"今天天气怎么样+提醒我开会"），才调用GLM进行意图分类。
